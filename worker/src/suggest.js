...
/**
 * ツチノート API — AI提案モジュール
 * Gemini 2.0 Flash で天気×作物の作業提案を生成
 */

import { generateUlid, jsonResponse, errorResponse } from './index.js';
import { getForecast } from './weather.js';
import { calculateStage } from './crop.js';

/** 今日の提案を取得 GET /api/suggestions/:farmId */
export async function handleSuggestions(userId, farmId, env) {
  // 畑の所有権チェック
...
/** 提案履歴 GET /api/suggestions/:farmId/history */
export async function handleSuggestionHistory(userId, farmId, env) {
  const farm = await env.DB.prepare(
